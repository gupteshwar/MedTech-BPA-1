...
def get_conditions(filters):
	...
	if filters.warehouse:
		if len(filters.warehouse) > 1:
			w = []
			for i in filters.warehouse:
				lft, rgt = frappe.db.get_value("Warehouse", i , ["lft", "rgt"])
				query = frappe.db.sql("""select name from `tabWarehouse` where lft >= %s and rgt <= %s """ % (lft,rgt,),as_dict =1)[0]
				w.append(query['name'])
			conditions += ("""and warehouse in {0}""".format(tuple(w)))
		else:	
			for i in filters.warehouse:
				lft, rgt = frappe.db.get_value("Warehouse", i , ["lft", "rgt"])
				conditions += (
						"""and warehouse in (select name from `tabWarehouse` where lft >= %s and rgt <= %s and sle.warehouse = name)""" % (lft,rgt,)
					)	
...
def get_items(filters):
	...
	print("\n\n\nitem code - ", filters.item_group, "\n\n\n")	
	if filters.get("item_group"): 
		conditions.append("item.item_group in %(item_group)s")	
