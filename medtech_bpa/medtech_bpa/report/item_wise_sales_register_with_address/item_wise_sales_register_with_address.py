	...
	for d in item_list:
		...
		row = {
			...
			**get_values_for_columns(additional_table_columns, d),
			...
			"address_line2": d.address_line2,	#added
		}
...
def get_columns(additional_table_columns, filters):
	...
	columns.extend(
		[
			...
			{"label": _("Ref No"), "fieldname": "address_line2", "fieldtype": "Data", "width": 150},	#Added
			{
				"label": _("Invoice"),
				"fieldname": "invoice",
				"fieldtype": "Link",
...
def get_items(filters, additional_query_columns, additional_conditions=None):
	...
	query = (
		...
		.left_join(address)
		.on(si.customer_address == address.name)
		.select(
			sii.name,
			sii.parent,
			si.posting_date,
			...
			sii.uom,
			sii.qty,
			address.address_line2.as_("address_line2"),
		)
